...
val rejectedtablename = s"supplychain_stage.${rejected_object_name}"
...
val filelog_process_runlog_check= spark.sql(s"select service_category, service_name, business_name, object_name,file_name, column_count, max(raw_load_datetime) as max_raw_load_datetime, max(load_timestamp)as max_load_datetime from tb_process_runlog_check where service_category like 'Data_Quality' and service_name like 'Data_Quality_Datatype' group by service_category, business_name, object_name,file_name, column_count,service_name")
...
// MAGIC %sql
// MAGIC select * from tb_auditlog_check

// COMMAND ----------

//Extract Latest Column Validation Values
val validate_dQ_datatype_status = spark.sql(s"""select trim(status) from tb_auditlog_check where service_category like 'Data_Quality' and service_name like 'Data_Quality_Datatype' and business_name = "$business_name" and object_name = "$object_name" and filename = "$filename" ORDER BY load_datetime DESC LIMIT 1""")

val colVal_status = if (!validate_dQ_datatype_status.rdd.isEmpty()) validate_dQ_datatype_status.head(1)(0)(0) else null
...
val val_final_failed = df_join.select(
  ...
  ,col("FileName")
  ,col("ColumnCount")
  ,lit(s"Failure in Data_Quality_Datatype : Check the Audit Log Table: $audit_log").as("Comment")
  ,col("LoadFormat")
...
val val_final = df_join.select(
  ...
  ,col("FileName")
  ,col("ColumnCount")
  ,lit("Data_Quality_Datatype : Completed Successfully").as("Comment")
  ,col("LoadFormat")
  ,col("LoadedTimeStamp").as("RawFileLoadedTimeStamp")
).withColumn("ProcessRunLoadTimestamp",lit(current_timestamp())).
withColumn("PipelineName", lit(s"$pipeline_name")).
...
// MAGIC %sql
// MAGIC select * from tb_val_final_failed

// COMMAND ----------

//Validation Check Entry on ProcessRunlog
if (colVal_status =="Pass") spark.table("tb_val_final").write.mode(SaveMode.Append).jdbc(jdbcUrl, s"$process_runlog", connectionProperties) else spark.table("tb_val_final_failed").write.mode(SaveMode.Append).jdbc(jdbcUrl, s"$process_runlog", connectionProperties)

// COMMAND ----------

//Validation Check
if (colVal_status =="Pass") "Validation Pass" else dbutils.notebook.exit("Column Mismatch -> Validation Failed- Job Exit")