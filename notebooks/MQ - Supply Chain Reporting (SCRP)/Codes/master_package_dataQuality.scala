...
// DBTITLE 1,dataQuality_dt_r2s
val dqdt_result_path = dbutils.notebook.run("/MQ - Supply Chain Reporting (SCRP)/Codes/dataQuality_dt_r2s/", 300, Map("source_layer" -> s"$source_layer", "business_name" -> s"$business_name", "loaded_timeStamp" -> s"$loaded_timeStamp", "source_name" -> s"$source_name", "object_name" -> s"$object_name", "stage_path" -> s"$stage_path", "stage_object_name" -> s"$stage_object_name", "rejected_path" -> s"$rejected_path","rejected_object_name" -> s"$rejected_object_name", "raw_object_name" -> s"$raw_object_name","landingrawpath" -> s"$landingrawpath", "srvc_control_table" -> s"$srvc_control_table","src_control_table" -> s"$src_control_table", "adls_storage_account_name" -> s"$adls_storage_account_name", "adls_container_name" -> s"$adls_container_name","file_format" -> s"$file_format", "filename" -> s"$filename", "filelog_tablename" -> s"$filelog_tablename", "pipeline_name" -> s"$pipeline_name","pipeline_runid" -> s"$pipeline_runid","pipeline_starttime" -> s"$pipeline_starttime", "column_count" -> s"$column_count", "process_runlog" -> s"$process_runlog", "audit_log" -> s"$audit_log", "scratchpad_dqdt_obj_path" -> s"$scratchpad_dqdt_obj_path"))

// COMMAND ----------

// DBTITLE 1,dataQuality_primaryKeyCheck
dbutils.notebook.run("/MQ - Supply Chain Reporting (SCRP)/Codes/dataQuality_pk_r2s/", 1000, Map("source_layer" -> s"$source_layer", "business_name" -> s"$business_name", "loaded_timeStamp" -> s"$loaded_timeStamp", "source_name" -> s"$source_name", "object_name" -> s"$object_name", "stage_path" -> s"$stage_path", "stage_object_name" -> s"$stage_object_name", "rejected_path" -> s"$rejected_path","rejected_object_name" -> s"$rejected_object_name", "raw_object_name" -> s"$raw_object_name","landingrawpath" -> s"$landingrawpath", "srvc_control_table" -> s"$srvc_control_table","src_control_table" -> s"$src_control_table", "adls_storage_account_name" -> s"$adls_storage_account_name", "adls_container_name" -> s"$adls_container_name","file_format" -> s"$file_format", "filename" -> s"$filename", "filelog_tablename" -> s"$filelog_tablename", "pipeline_name" -> s"$pipeline_name","pipeline_runid" -> s"$pipeline_runid","pipeline_starttime" -> s"$pipeline_starttime", "column_count" -> s"$column_count", "process_runlog" -> s"$process_runlog", "audit_log" -> s"$audit_log","scratchpad_dqdt_obj_path" -> s"$scratchpad_dqdt_obj_path","scratchpad_dqpk_obj_path" -> s"$scratchpad_dqpk_obj_path"))