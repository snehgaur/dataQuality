...
val filelog_process_runlog_check= spark.sql(s"select service_category, service_name, business_name, object_name,file_name, column_count, max(raw_load_datetime) as max_raw_load_datetime, max(load_timestamp)as max_load_datetime from tb_process_runlog_check where service_category like 'Data_Cleansing' and service_name like 'Data_Cleansing_Dedup' group by service_category, business_name, object_name,file_name, column_count,service_name")
...
val dqpk_result_path = dbutils.notebook.run("/MQ - Supply Chain Reporting (SCRP)/Services/dataCleansing_dedupService/", 1500, Map("process_runlogId" -> s"$process_runlogId","business_name" -> s"$business_name", "object_name" -> s"$object_name", "control_table" -> s"$srvc_control_table", "adls_storage_account_name" -> s"$adls_storage_account_name", "adls_container_name" -> s"$adls_container_name", "rawtablename" -> s"$rawtablename", "stagetablename" -> s"$stagetablename", "rejectedtablename" -> s"$rejectedtablename", "audit_log" -> s"$audit_log", "src_control_table" -> s"$src_control_table","srvc_control_table" -> s"$srvc_control_table","process_runlog" -> s"$process_runlog","raw_filter" -> s"$loaded_timeStamp","raw_filter_filename" -> s"$filename","column_count" -> s"$column_count","scratchpad_dctm_obj_path" -> s"$scratchpad_dctm_obj_path","scratchpad_dcdd_obj_path" -> s"$scratchpad_dcdd_obj_path"))
...
val val_final = df_join.select(
  ...
  ,lit("Data_Cleansing_Dedup : Completed Successfully").as("Comment")
