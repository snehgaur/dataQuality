...
val filelog_process_runlog_check= spark.sql(s"select service_category, service_name, business_name, object_name,file_name, column_count, max(raw_load_datetime) as max_raw_load_datetime, max(load_timestamp)as max_load_datetime from tb_process_runlog_check where service_category like 'Data_Cleansing' and service_name like 'Data_Cleansing_NotNull' group by service_category, business_name, object_name,file_name, column_count,service_name")

// COMMAND ----------

val val_filter = filelog_process_runlog_check.select(col("max_raw_load_datetime"))  //What is Max Timestamp in the Intermediate Control Table
...
// MAGIC %sql
// MAGIC select * from df_final_frame

// COMMAND ----------

val temp_filename = spark.sql(s"""select file_name from df_final_frame where file_name = "${filename}" group by file_name""")
...
// DBTITLE 1,Data Cleaning Not Null Service Call
val result = dbutils.notebook.run("/MQ - Supply Chain Reporting (SCRP)/Services/dataCleansing_notNull/", 1500, Map("process_runlogId" -> s"$process_runlogId","business_name" -> s"$business_name", "object_name" -> s"$object_name", "control_table" -> s"$srvc_control_table", "adls_storage_account_name" -> s"$adls_storage_account_name", "adls_container_name" -> s"$adls_container_name", "rawtablename" -> s"$rawtablename", "stagetablename" -> s"$stagetablename", "rejectedtablename" -> s"$rejectedtablename", "audit_log" -> s"$audit_log", "src_control_table" -> s"$src_control_table","srvc_control_table" -> s"$srvc_control_table","process_runlog" -> s"$process_runlog","raw_filter" -> s"$loaded_timeStamp","raw_filter_filename" -> s"$filename","column_count" -> s"$column_count","scratchpad_source_path" -> s"$scratchpad_source_path","scratchpad_target_path" -> s"$scratchpad_target_path"))
...
val val_final = df_join.select(
  ...
  ,lit("Data_Cleansing_Trim : Completed Successfully").as("Comment")
...
//Insert in ProcessRunlog
spark.table("tb_val_final").write.mode(SaveMode.Append).jdbc(jdbcUrl, s"$process_runlog", connectionProperties)

// COMMAND ----------

dbutils.notebook.exit(s"Data Cleansing Trim Service completed for $business_name: $object_name")