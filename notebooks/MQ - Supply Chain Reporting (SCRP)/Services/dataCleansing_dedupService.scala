...
dbutils.widgets.text("scratchpad_dctm_obj_path","")
val scratchpad_dctm_obj_path = dbutils.widgets.get("scratchpad_dctm_obj_path")
dbutils.widgets.text("scratchpad_dcdd_obj_path","")
val scratchpad_dcdd_obj_path = dbutils.widgets.get("scratchpad_dcdd_obj_path")
...
val rawdataframe= sqlContext.read.parquet(scratchpad_dctm_obj_path)
rawdataframe.count
...
val val_metaDataList_DeDupCol = (df_DataOps_Control.select("ColumnName", "DeDupCol")).where(col("BusinessName").equalTo(s"$business_name") && col("ObjectName").equalTo(s"$object_name") && col("DeDupCol").isin("Y"))
val metaDataList_DeDupColByList = val_metaDataList_DeDupCol.collect().map(_.getString(0)).toArray

// COMMAND ----------

val metaDataList_DeDupOrderByList = df_DataOps_Control.select("ColumnName", "DeDupOrderFlag","DeDupOrderRank").where(col("BusinessName").equalTo(s"$business_name") && col("ObjectName").equalTo(s"$object_name")&& col("DeDupOrderFlag").isin("Asc", "Desc"))
...
val dfDuplicate_final = dfDuplicate_mdr1.union(dfDuplicate)

// COMMAND ----------

val cleaned_finalset =  dfwithoutDuplicate.drop("load_datetime").drop("datepart").drop("Status").drop("ServiceCode").drop("count").withColumn("ServiceCode",lit("3")).withColumn("Status",lit("Pass")).withColumn("load_datetime",lit(current_timestamp())).withColumn("datepart",lit(current_date())).drop("rank")
...
cleaned_finalset.write.mode(SaveMode.Overwrite).format("parquet").save(s"$scratchpad_dcdd_obj_path")

// COMMAND ----------

display(dfDuplicate_final)

// COMMAND ----------

val rejected_finalset =  dfDuplicate_final.drop("load_datetime").drop("datepart").drop("Status").drop("ServiceCode").drop("count").withColumn("ErrorMapCol",col("rank")).withColumn("ServiceCode",lit("3")).withColumn("Status",lit("Fail")).withColumn("load_datetime",lit(current_timestamp())).withColumn("datepart",lit(current_date())).drop("rank")
...
val auditlog_df_pass = dfwithoutDuplicate.select(
  ...
  ,lit("Data_Cleansing_Dedup").as("ServiceName")
  ,lit(s"$raw_filter_filename").as("FileName")
  ,lit("Pass").as("Status")
  ,concat(lit("Pass"),lit(s" in Data Cleansing Dedup for $object_name")).as("Description")
...
,lit("dedupService").as("service_name")
,lit("stage").as("storage_layer")
,lit("Pass").as("status")  
,lit("Dedup Service is Completed").as("description")
...
val auditlog_df_rejected = dfDuplicate_final.select(
   lit(s"$process_runlogId").as("ProcessRunlogId")
  ,lit(s"$business_name").as("BusinessName")
  ,lit(s"$object_name").as("ObjectName")
  ,lit("Stage").as("StorageLayer")
  ...
  ,lit(s"$raw_filter_filename").as("FileName")
  ,lit("Fail").as("Status")
  ,concat(lit("Fail"),lit(s" in Data Cleansing Dedup for $object_name")).as("Description")
  ,lit(s"$column_count").as("SourceColCount")
).groupBy("ProcessRunlogId","BusinessName","ObjectName","StorageLayer","ServiceCategory","ServiceName","FileName","Status","Description","SourceColCount").count().withColumn("LoadTimestamp",lit(current_timestamp())).withColumn("Datepart",lit(current_date()))
...
,lit("stage").as("storage_layer")
,lit("Fail").as("status")  
,lit("Fail in Dedup Service").as("description")
).groupBy("business_name","object_name","service_name","storage_layer","status","description").count().as("result_count").
withColumn("load_datetime",lit(current_timestamp())).withColumn("datepart",lit(current_date()))
*/
...
val auditlog_df = auditlog_df_pass.union(auditlog_df_rejected)

// COMMAND ----------

val df_audittablename = auditlog_df.select(col("ProcessRunlogId"),
...
// MAGIC %sql
// MAGIC select * from tb_audittablename
...
dbutils.notebook.exit(s"$scratchpad_dcdd_obj_path")