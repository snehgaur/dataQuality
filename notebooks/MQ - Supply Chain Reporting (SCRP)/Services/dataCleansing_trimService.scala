...
val rawdataframe= sqlContext.read.parquet(scratchpad_dqpk_obj_path)
rawdataframe.count
...
// DBTITLE 1,Trim Logic
//null_check_list
val val_metaDataList_trimList = (df_DataOps_Control.select("ColumnName", "Trim")).where(col("BusinessName").equalTo(s"$business_name") && col("ObjectName").equalTo(s"$object_name") && col("Trim").isin("Y"))
...
val cleaned_finalset =  trimmedDf.drop("load_datetime").drop("datepart").drop("Status").drop("ServiceCode").drop("count").withColumn("ServiceCode",lit("3")).withColumn("Status",lit("Pass")).withColumn("load_datetime",lit(current_timestamp())).withColumn("datepart",lit(current_date()))
...
cleaned_finalset.write.mode(SaveMode.Overwrite).format("parquet").save(s"$scratchpad_dctm_obj_path")

// COMMAND ----------

val df_audittablename_val = trimmedDf.select(
  ...
  ,lit("Pass").as("Status")
  ,concat(lit("Pass"),lit(s" in Data Cleansing Trim for $object_name")).as("Description")
...
val df_audittablename = df_audittablename_val.select(col("ProcessRunlogId"),
...
dbutils.notebook.exit(s"$scratchpad_dctm_obj_path")