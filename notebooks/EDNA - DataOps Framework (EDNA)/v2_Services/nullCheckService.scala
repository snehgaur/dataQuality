...
//null_check_list
val val_metaDataList_notNullList = (df_DataOps_Control.select("ColumnName", "nullCheck")).where(col("BusinessName").equalTo(s"$BusinessName") && col("ObjectName").equalTo(s"$ObjectName") && col("nullCheck").isin("Y"))
//val metaDataList_DeDupColByList = val_metaDataList_DeDupCol.collect().map(_.getString(0)).toArray

// COMMAND ----------

val metaDataList_notNullList = val_metaDataList_notNullList.orderBy("nullCheck").collect.toList
