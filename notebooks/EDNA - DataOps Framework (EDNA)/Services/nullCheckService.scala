...
//null_check_list
val val_metaDataList_notNullList = (df_DataOps_Control.select("ColumnName", "NotNullCheck")).where(col("BusinessName").equalTo(s"$BusinessName") && col("ObjectName").equalTo(s"$ObjectName") && col("NotNullCheck").isin("Y"))
